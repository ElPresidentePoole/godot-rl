...
func perform(parent: Node) -> void:
	var pos_final: Vector2i = parent.cellmap.node_to_cell_pos_map[actor] + dv
	if parent.astar.is_point_disabled(parent.astar.get_closest_point(pos_final, true)):
		...
		var world_pos_final: Vector2 = parent.cellmap.cell_pos_to_world(pos_final)
		await actor.play_movement_tween(world_pos_final)
		parent.cellmap.node_to_cell_pos_map[actor] = pos_final
